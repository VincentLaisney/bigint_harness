import rand
import hanabi1224.biginteger as big
import time
import os
...
fn bigint_str_to_arr(str_arr []string) []big.BigInteger  {
	mut res := []big.BigInteger{}
	for i:=0; i<str_arr.len; i++ {
		stri:=str_arr[i]
		z := big.from_str(stri) or { big.zero }
		res << z
	}
	return res
}

fn do_calculation(a big.BigInteger, b big.BigInteger, op byte) big.BigInteger {
	mut result := big.BigInteger{}
	match op {
	`+`
		{ result = a + b }
	`-`
...
fn do_calc_and_print(a big.BigInteger, b big.BigInteger, op byte) big.BigInteger {
...
fn main()  {
	if os.args.len > 1 {
		...
		for line in rnd_args {
			...
			op := line[1]
			b := line[2]
			a_bi := big.from_str(a) or { big.zero }
			b_bi := big.from_str(b) or { big.zero }
